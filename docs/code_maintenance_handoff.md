...
- 将“设计用RTM”中 FR-004 从单行拆分为“知识库创建”“知识库查看”“知识库编辑”“知识库删除”“向量集合与关联清理”5 个小功能行。
- 将 FR-017 从单行拆分为“文档集创建与维护”“文档维护与版本快照”“文档转数据集”3 个小功能行。
- 每个拆分后的小功能均补充独立详细说明，担当者和责任者均设置为“杨帆”，并保留原有 PD/COD/UT 状态。
- 更新 `tests/test_rtm_workbook.py`，校验 FR-004 和 FR-017 的拆分行数、功能点名称、说明完整性和负责人一致性。

### 3.22 浅色工作空间界面改版

- 以暖白背景、墨绿主色和文字侧栏重构主工作空间，默认入口调整为知识库，并保留浅色/深色主题切换。
- 知识库与文档库使用共用列表页面，支持搜索、创建时间或名称排序、网格/列表视图、加载骨架、失败重试、空状态和使用指南；全部展示数据继续来自现有接口。
- 卡片改为语义化按钮与稳定配色，显示真实创建时间，并保留进入、编辑、删除流程；对话、搜索、设置页同步补充欢迎态、标题层级和响应式样式。
- 新增 `library-view` 前端逻辑测试与浏览器验收夹具，覆盖搜索、排序、原数组不变、日期格式、稳定配色和状态交互。
- 仅创建本地 Git 提交，未推送到 GitHub。

### 3.23 浅色界面变更回写需求跟踪矩阵

- 在最终需求跟踪矩阵“变更履历”中新增第 23、24 条记录，分别追踪浅色工作空间界面提交 `2431eca` 和本次 RTM 回写；变更人均为李子昂。
- 在“设计用RTM”中补充 FR-004、FR-014、FR-016、FR-017、NFR-001、NFR-005、NFR-007 的界面实现与验证信息，并将这些变更项的担当者更新为李子昂；原责任者保持不变。
- 汇总行移动至第 27 行，需求总数仍为 25，需求变更比汇总公式扩展为 `=SUM(M3:M26)`。
- 更新 `tests/test_rtm_workbook.py`，覆盖 24 条履历、新增记录、担当者、实现关键词和汇总公式。
...
2026-09-14 在浅色界面提交 `2431eca` 基础上更新 `outputs/rtm-maintenance-final/` 中唯一需求跟踪矩阵，并同步修改 RTM 回归测试和本维护记录。模型权重、数据库、上传资料及其他运行数据均未纳入提交。

本机已配置 `.venv` 与 `wenkb-client/node_modules`；前端开发地址为 `http://127.0.0.1:11420/`，后端健康检查为 `http://127.0.0.1:16088/health`。后台预览进程只用于本地验收，不属于提交内容。
...
```powershell
.venv\Scripts\python.exe -m unittest tests.test_rtm_workbook
```

结果为 RTM 专项测试 8 项通过、全量 Python 测试 46 项通过。另使用 Artifact Tool 重新计算并扫描公式错误，结果为 0 个匹配；重新渲染“设计用RTM”和“变更履历”，确认新增内容、姓名、日期、行高、日期格式和汇总行显示正常。
...
结果为前端逻辑测试 6 项通过、生产构建成功、后端与文档全量测试 45 项通过。另在真实本地服务与独立测试夹具中验证知识库数据加载、搜索与排序、网格/列表切换、加载失败重试、空状态、编辑入口、键盘操作以及 1440/900/700 像素断点；页面控制台无错误、窄屏无横向溢出。构建仍保留项目既有的 `eval`、源码映射和大体积 chunk 告警。
...
2026-09-08 本轮需求跟踪矩阵功能点拆行已完成以下验证：

```powershell
python -m unittest tests.test_rtm_workbook
```

结果为通过。另重新渲染“设计用RTM”，确认拆分后行样式、状态底色和底部说明行显示正常；Artifact Tool 公式错误扫描结果为 0 个匹配。
...
- 2026-09-14：将浅色工作空间界面改版回写到最终需求跟踪矩阵，在“变更履历”新增第 23、24 条记录，并在“设计用RTM”更新 7 个关联需求的实现说明和担当者李子昂；同步扩展 RTM 回归测试并完成公式错误扫描及两张工作表的渲染检查。

- 2026-09-13：完成方案 A 浅色工作空间界面改版，新增共用知识库/文档库列表、语义化卡片、搜索排序、响应式断点、加载与空状态、使用指南，并统一对话、搜索和设置页视觉；新增 6 项前端逻辑测试和浏览器验收夹具，完成生产构建、45 项 Python 全量回归及真实本地服务验收；变更仅保存在本地提交，未推送 GitHub。
...
- 2026-09-08：继续细化需求跟踪矩阵“设计用RTM”，将 FR-004 和 FR-017 的宽泛小分类拆成独立功能点行，每行补充详细说明并统一担当者、责任者为杨帆；更新 RTM 回归测试并完成渲染与公式错误扫描验证。
